...
class YmazeDrawer(DefaultDrawer):
    ...
    def _annotate_frame(self,img, positions, tracking_units):
        ...
        if self._holes is not None:
            holes = self._holes
            cv2.drawKeypoints(img, holes, np.array([]), (0,0,255), cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
        ...
        for track_u in tracking_units:
            ...
            x_center =track_u.roi.rectangle[0] + track_u.roi.rectangle[2]/2
            y_center =track_u.roi.rectangle[1] + track_u.roi.rectangle[3]/2

            cv2.circle(img, (int(x_center),int(y_center)),1, (0,0,255), 1, cv2.CV_AA)
            cv2.circle(img, (int(x_center),int(y_center)),112, (0,0,255), 1, cv2.CV_AA)
            cv2.circle(img, (int(x_center),int(y_center)),127, (0,0,255), 1, cv2.CV_AA)
            cv2.circle(img, (int(x_center),int(y_center)),145, (0,255,0), 1, cv2.CV_AA)
            cv2.circle(img, (int(x_center),int(y_center)),160, (0,255,0), 1, cv2.CV_AA)
            cv2.circle(img, (int(x_center),int(y_center)),178, (255,0,0), 1, cv2.CV_AA)
            cv2.circle(img, (int(x_center),int(y_center)),193, (255,0,0), 1, cv2.CV_AA)
            cv2.circle(img, (int(x_center),int(y_center)),211, (0,0,255), 1, cv2.CV_AA)
            cv2.circle(img, (int(x_center),int(y_center)),226, (0,0,255), 1, cv2.CV_AA)
            cv2.circle(img, (int(x_center),int(y_center)),244, (0,255,0), 1, cv2.CV_AA)
            cv2.circle(img, (int(x_center),int(y_center)),259, (0,255,0), 1, cv2.CV_AA)
            cv2.circle(img, (int(x_center),int(y_center)),277, (255,0,0), 1, cv2.CV_AA)
            #cv2.circle(img, (int(x_center),int(y_center)),292, (0,255,0), 1, cv2.CV_AA)



            print track_u.roi.rectangle
            print track_u.roi.rectangle[0]
            print track_u.roi.rectangle[1]
            print track_u.roi.rectangle[2]
            try:
...
class YmazeROIBuilder(TargetGridROIBuilder):
    ...
    def _rois_from_img(self,img):
        ...
        self._holes = self._find_holes(img)
        all_holes = self._holes
        ...
        for i,r in enumerate(rectangles):
            ...
            cv2.imshow("dbg",img)
            cv2.waitKey(0)
        return rois, sorted_src_pts, all_holes
    ...
    def build(self, input):
        ...
        try:
            # Detect blobs.
            rois, sorted_targets, all_holes = self._rois_from_img(accum)
        ...
        return rois, sorted_targets, all_holes
...
if __name__ == "__main__":
    ...
    rois, sorted_targets, holes = roi_builder.build(cam)
    ...
    drawer = YmazeDrawer(video_out='/home/diana/Desktop/example_video3.avi', draw_frames=True, targets=sorted_targets,holes=holes)
