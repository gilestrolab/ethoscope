...
class AsyncMySQLWriter(BaseAsyncSQLWriter):
    ...
    def _delete_my_sql_db(self):
        ...
        try:
            logging.info(
                f"Attempting to connect to mysql db {self._db_name} on host {self._db_host} as {self._db_user_name}"
            )
        ...
        try:
            ...
            if log_bin_status and log_bin_status[1] == "ON":
                logging.info(
                    "The binary logs are set to true. Resetting them to save space."
                )
    ...
    def _should_retry_on_error(self, error):
        ...
        is_critical = any(
            critical_error in error_str for critical_error in critical_errors
        )
...
class MySQLResultWriter(BaseResultWriter):
    ...
    def _create_async_writer(self, db_credentials, erase_old_db, **kwargs):
        ...
        return self._async_writing_class(
            db_credentials, self._queue, erase_old_db, self._db_host
        )
    ...
    def _create_all_tables(self):
        ...
        if self._erase_old_db:
            ...
            self._create_table(
                "ROI_MAP",
                "roi_idx SMALLINT, roi_value SMALLINT, x SMALLINT,y SMALLINT,w SMALLINT,h SMALLINT",
            )
            for r in self._rois:
            ...
            self._create_table(
                "VAR_MAP",
                "var_name CHAR(100), sql_type CHAR(100), functional_type CHAR(100)",
            )
            if self._shot_saver is not None:
                logging.info("Creating table for IMG_screenshots")
                self._create_table(
                    self._shot_saver.table_name, self._shot_saver.create_command
                )
            ...
            self._create_table(
                "START_EVENTS",
                "id INT  NOT NULL AUTO_INCREMENT PRIMARY KEY, t INT, event CHAR(100)",
            )
