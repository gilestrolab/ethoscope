...
class ROITemplateManager:
    ...
    def __init__(
        ...
        if builtin_templates_dir is None:
            self.builtin_dir = os.path.abspath(
                os.path.join(
    ...
    def _ensure_directories(self):
        ...
        try:
            ...
            self.custom_dir = os.path.join(
                tempfile.gettempdir(), "ethoscope_roi_templates"
            )
            os.makedirs(self.custom_dir, exist_ok=True)
    ...
    def load_template(self, name: str) -> ROITemplate:
        ...
        raise FileNotFoundError(
            f"Template '{name}' not found in builtin or custom directories"
        )
...
def convert_legacy_to_template(legacy_builder_class, **legacy_params) -> Dict[str, Any]:
    ...
    if (
        "TargetGrid" in class_name
        or "SleepMonitor" in class_name
        or "Olfaction" in class_name
        or "ElectricShock" in class_name
    ):
        ...
        template_data["roi_definition"] = {"type": "manual_polygons", "manual_rois": []}
