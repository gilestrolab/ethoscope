...
def mock_camera():
    ...
    camera.__iter__ = Mock(return_value=iter([
        (0, Mock()),      # t=0ms
        (1000, Mock()),   # t=1000ms  
        (2000, Mock()),   # t=2000ms
        (3000, Mock()),   # t=3000ms
...
def sample_rois():
    ...
    roi1 = Mock(spec=ROI)
    roi1.idx = 1
    roi1.get_feature_dict = Mock(return_value={
        "idx": 1, "value": 255, "x": 10, "y": 10, "w": 100, "h": 100
    })
    
    roi2 = Mock(spec=ROI)
    roi2.idx = 2
    roi2.get_feature_dict = Mock(return_value={
        "idx": 2, "value": 255, "x": 120, "y": 10, "w": 100, "h": 100
    })
...
class TestMonitorTimeOffset:
    ...
    def test_monitor_init_with_time_offset(self, mock_camera, mock_tracker_class, sample_rois):
    ...
    def test_monitor_time_offset_applied_to_timestamps(self, mock_camera, mock_tracker_class, sample_rois):
    ...
    def test_monitor_last_time_stamp_property(self, mock_camera, mock_tracker_class, sample_rois):
...
class TestSQLiteAppendFunctionality:
    ...
    def test_sqlite_get_last_timestamp_missing_roi_table(self, sample_rois):
        ...
        try:
            ...
            cursor.execute("INSERT INTO ROI_1 (t, x, y) VALUES (?, ?, ?)", (8000, 10.5, 20.5))
...
class TestAppendIntegration:
    ...
    def test_monitor_with_append_offset(self, mock_camera, mock_tracker_class, sample_rois):
