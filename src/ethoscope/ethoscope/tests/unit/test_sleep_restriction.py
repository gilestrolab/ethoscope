...
class TestmAGOSleepRestriction(unittest.TestCase):
    ...
    def test_init_with_defaults(self):
        ...
        stimulator = mAGOSleepRestriction(
            hardware_connection=self.mock_hardware, state_dir=self.temp_dir
        )
    ...
    def test_bind_tracker_creates_state_file(self):
        ...
        expected_state_file = os.path.join(
            self.temp_dir, "sleep_restriction_roi_1.json"
        )
        self.assertEqual(stimulator._state_file_path, expected_state_file)
        self.assertEqual(
            stimulator._daily_scheduler._state_file_path, expected_state_file
        )
    ...
    def test_apply_during_active_period(self, mock_time):
        ...
        with patch.object(
            stimulator.__class__.__bases__[0], "apply"
        ) as mock_parent_apply:
