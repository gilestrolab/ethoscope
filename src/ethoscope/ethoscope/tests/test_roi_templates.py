...
class TestROITemplate(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.valid_template_data = {
            ...
            "roi_definition": {
                ...
                "grid": {"n_rows": 2, "n_cols": 2, "orientation": "vertical"},
                "alignment": {"target_detection": True, "expected_targets": 3},
                "positioning": {
                    "margins": {"top": 0.1, "bottom": 0.1, "left": 0.1, "right": 0.1},
                    "fill_ratios": {"horizontal": 0.8, "vertical": 0.8},
                },
            },
        }

        self.manual_template_data = {
            "template_info": {"name": "Manual Test Template", "version": "1.0"},
            "roi_definition": {
                "type": "manual_polygons",
                "manual_rois": [
                    {"polygon": [[0, 0], [100, 0], [100, 100], [0, 100]], "value": 0},
                    {
    ...
    def test_manual_template_validation(self):
        ...
        invalid_data2["roi_definition"]["manual_rois"][0]["polygon"] = [
            [0, 0],
            [100, 0],
        ]
        with self.assertRaises(ROITemplateValidationError):
...
class TestFileBasedROIBuilder(unittest.TestCase):
    ...
    def setUp(self):
        """Set up test fixtures."""
        self.template_data = {
            "template_info": {"name": "Test Builder Template", "version": "1.0"},
            "roi_definition": {
                "type": "manual_polygons",
                "manual_rois": [
                    {"polygon": [[10, 10], [50, 10], [50, 50], [10, 50]], "value": 0}
                ],
            },
        }

        self.mock_camera = Mock()
        self.mock_camera.grab_frame.return_value = np.zeros(
    ...
    def test_create_with_multiple_sources(self):
        ...
        with self.assertRaises(ValueError):
            FileBasedROIBuilder(
                template_file="test.json", template_data=self.template_data
            )
...
class TestROITemplateManager(unittest.TestCase):
    ...
    def test_save_and_load_template(self):
        ...
        # Save template
        saved_path = self.manager.save_template(
            "test_template", self.sample_template, "custom"
        )
...
class TestLegacyMigration(unittest.TestCase):
    ...
    def test_convert_legacy_to_template(self):
        ...
        self.assertEqual(
            template_data["roi_definition"]["positioning"]["margins"]["top"], 0.1
        )
        self.assertEqual(
            template_data["roi_definition"]["positioning"]["fill_ratios"]["horizontal"],
            0.8,
        )
...
class TestTemplateIntegration(unittest.TestCase):
    ...
    def test_end_to_end_workflow(self):
        ...
        template_data = {
            ...
            "validation": {"min_roi_area": 100},
        }

        # Save template
        saved_path = self.manager.save_template(
            "integration_test", template_data, "custom"
        )
        self.assertTrue(os.path.exists(saved_path))
