...
def send_command(
    ...
    message = {"command": action, "data": data}
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        if not response:
            raise OSError("Received empty response from server")
        ...
        if not response_str:
            raise OSError("Received empty response after decoding")
        ...
        try:
            ...
            print(
                f"Failed to parse JSON response: '{response_str[:100]}...' Error: {e}"
            )
...
if __name__ == "__main__":
    ...
    parser.add_option(
        ...
        dest="host",
        default="127.0.0.1",
        help="The IP of the ethoscope to be interrogated",
    )
    parser.add_option(
        "-c",
        "--command",
        dest="command",
        default="status",
        help="The command to be sent. Send help to receive a list of available commands.",
    )
    parser.add_option(
        "-d", "--data", dest="data", default="", help="dictionary with data to be sent"
    )
    parser.add_option(
        "-t",
        "--timing",
