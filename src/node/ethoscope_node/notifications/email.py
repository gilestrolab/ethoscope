...
class EmailNotificationService(NotificationAnalyzer):
    ...
    def _should_send_alert(
        ...
        """De-dup an alert via the persistent ``alert_logs`` table.

        Idempotency is keyed on ``(device_id, alert_type, run_id)``: the scanner
        will only call us once per run finalisation, but a polling-thread race
        could in principle produce two near-simultaneous invocations, and the
        DB key catches that. The previous in-memory cooldown layer (1h between
        "similar" alerts) was redundant with this and produced silent
        false-negatives when ``run_id`` differed but the cooldown window
        overlapped, so it is gone.
        """
