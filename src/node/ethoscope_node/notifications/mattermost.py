...
class MattermostNotificationService(NotificationAnalyzer):
    ...
    def _send_message(
        ...
        if not all([server_url, bot_token, channel_id]):
            self.logger.error(
                "Mattermost configuration incomplete - missing server_url, bot_token, or channel_id"
            )
            return False
        ...
        try:
            ...
            self.logger.info(
                f"Mattermost message sent successfully to channel {channel_id}"
            )
            return True
        ...
        try:
            ...
            if failure_analysis.get("experiment_duration_str"):
                message_parts.append(
                    f"**Duration:** {failure_analysis['experiment_duration_str']}"
                )
            ...
            if success:
                if message:  # Ensure message is not empty before logging
                    try:
                        self.db.logAlert(
                            device_id, "device_stopped", message, "mattermost", run_id
                        )
                    except Exception as e:
    ...
    def send_storage_warning_alert(
        ...
        try:
            message_parts = [
                ...
                "",
                "Please free up space or backup data to avoid experiment interruption.",
            ]
    ...
    def send_device_unreachable_alert(
        ...
        try:
            ...
            message_parts = [
                ...
                "",
                "Device may have network issues or be powered off.",
            ]
