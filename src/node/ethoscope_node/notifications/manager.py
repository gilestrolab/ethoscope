...
class NotificationManager(NotificationAnalyzer):
    ...
    def _initialize_services(self):
        ...
        try:
            ...
            if mattermost_config.get("enabled", False):
                try:
                    mattermost_service = MattermostNotificationService(
                        self.config, self.db
                    )
            ...
            self.logger.info(
                f"Notification manager initialized with {len(self._services)} active services"
            )
        ...
        for service_name, service in self._services:
            try:
                ...
                if result:
                    success_count += 1
                    self.logger.debug(
                        f"Device stopped alert sent successfully via {service_name}"
                    )
                else:
                    self.logger.warning(
                        f"Device stopped alert failed via {service_name}"
                    )
            except Exception as e:
                self.logger.error(
                    f"Error sending device stopped alert via {service_name}: {e}"
                )

        total_services = len(self._services)
        if success_count > 0:
            self.logger.info(
                f"Device stopped alert sent via {success_count}/{total_services} services"
            )
            return True
        else:
            self.logger.error(
                f"Device stopped alert failed in all {total_services} services"
            )
            return False
        ...
        for service_name, service in self._services:
            try:
                ...
                if result:
                    success_count += 1
                    self.logger.debug(
                        f"Storage warning sent successfully via {service_name}"
                    )
                else:
                    self.logger.warning(f"Storage warning failed via {service_name}")
            except Exception as e:
                self.logger.error(
                    f"Error sending storage warning via {service_name}: {e}"
                )

        total_services = len(self._services)
        if success_count > 0:
            self.logger.info(
                f"Storage warning sent via {success_count}/{total_services} services"
            )
            return True
        else:
            self.logger.error(
                f"Storage warning failed in all {total_services} services"
            )
            return False
        ...
        for service_name, service in self._services:
            try:
                result = service.send_device_unreachable_alert(
                    device_id=device_id, device_name=device_name, last_seen=last_seen
                )
                if result:
                    success_count += 1
                    self.logger.debug(
                        f"Device unreachable alert sent successfully via {service_name}"
                    )
                else:
                    self.logger.warning(
                        f"Device unreachable alert failed via {service_name}"
                    )
            except Exception as e:
                self.logger.error(
                    f"Error sending device unreachable alert via {service_name}: {e}"
                )

        total_services = len(self._services)
        if success_count > 0:
            self.logger.info(
                f"Device unreachable alert sent via {success_count}/{total_services} services"
            )
            return True
        else:
            self.logger.error(
                f"Device unreachable alert failed in all {total_services} services"
            )
            return False
    ...
    def test_all_configurations(self) -> Dict[str, Any]:
        ...
        for service_name, service in self._services:
            try:
                if (
                    hasattr(service, "test_email_configuration")
                    and service_name == "email"
                ):
                    result = service.test_email_configuration()
                elif (
                    hasattr(service, "test_mattermost_configuration")
                    and service_name == "mattermost"
                ):
                    result = service.test_mattermost_configuration()
                elif (
                    hasattr(service, "test_slack_configuration")
                    and service_name == "slack"
                ):
    ...
    def reload_configuration(self):
        ...
        self._initialize_services()
        self.logger.info(
            f"Configuration reloaded with {len(self._services)} active services"
        )
