...
class BaseSQLConnector:
    ...
    def __init__(
        ...
        remote_user: str = "ethoscope",
        remote_pass: str = "ethoscope",
        dst_path: str = None,
        ...
        with DatabaseConnectionManager(
            ...
            for db_name, table_name in tables:
                ...
                if table_name in self._TABLE_WITHOUT_KEY:
                    ...
                    query = (
                        f"SELECT COALESCE(MAX(id), 0) FROM `{db_name}`.`{table_name}`"
                    )

                try:
                    cursor.execute(query)
                    result = cursor.fetchone()
                    db_info[db_name][table_name] = (
    ...
    def compare_databases(self, use_fast_mode: bool = False) -> float:
        ...
        try:
            ...
            for table_name in all_tables:
                ...
                remote_count = 0
                if use_fast_mode:
                    remote_count = remote_tables_info.get("default", {}).get(
                        table_name, 0
                    )
                else:
                    db_name = self._remote_db_name or list(remote_tables_info.keys())[0]
                    remote_count = remote_tables_info.get(db_name, {}).get(
                        table_name, 0
                    )
            ...
            if match_percentage > 100.0:
                logging.warning(
                    f"Local database has more records than remote ({match_percentage:.2f}% - this may indicate duplicate data)"
                )
...
def get_backup_path_from_database(host, ethoscope_number=None):
    ...
    try:
        ...
        cursor.execute(
            "SELECT DISTINCT value FROM METADATA WHERE field = 'backup_filename' AND value IS NOT NULL"
        )
        result = cursor.fetchone()
...
class MySQLdbToSQLite(BaseSQLConnector):
    ...
    def __init__(
        self,
        dst_path: str,
        remote_db_name: str = "ethoscope_db",
        remote_host: str = "localhost",
        remote_user: str = "ethoscope",
        remote_pass: str = "ethoscope",
    ...
    def _ensure_table_schema(self, table_name: str, mysql_conn, sqlite_conn) -> bool:
        ...
        try:
            ...
            if table_exists:
                ...
                if self._table_has_proper_constraints(
                    table_name, sqlite_conn, table_schema
                ):
                    logging.debug(
                        f"Table {table_name} already exists with proper constraints"
                    )
                    return True
                else:
                    # Need to migrate existing table
                    logging.info(
                        f"Table {table_name} exists but lacks proper constraints, migrating..."
                    )
                    return self._migrate_table_schema(
                        table_name, sqlite_conn, table_schema
                    )
            else:
                # Create new table with proper constraints
                logging.debug(f"Creating new table {table_name} with constraints")
                return self._create_table_with_constraints(
                    table_name, sqlite_conn, table_schema
                )
    ...
    def _get_mysql_table_schema(self, table_name: str, mysql_conn) -> dict:
        ...
        try:
            ...
            schema = {"columns": [], "primary_key": None, "has_auto_increment": False}
    ...
    def _table_has_proper_constraints(
        self, table_name: str, sqlite_conn, expected_schema: dict
    ) -> bool:
        ...
        try:
            ...
            if expected_pk:
                ...
                for col_info in sqlite_columns:
                    ...
                    if col_name == expected_pk and is_pk:
                        logging.debug(
                            f"Table {table_name} has proper PRIMARY KEY constraint on {expected_pk}"
                        )
                        return True

                logging.debug(
                    f"Table {table_name} missing PRIMARY KEY constraint on {expected_pk}"
                )
                return False
            else:
            ...
            logging.error(
                f"Error checking SQLite table constraints for {table_name}: {e}"
            )
            return False

    def _create_table_with_constraints(
        self, table_name: str, sqlite_conn, schema: dict
    ) -> bool:
        ...
        try:
            ...
            logging.info(
                f"Successfully created table {table_name} with PRIMARY KEY constraints"
            )
            return True
    ...
    def _migrate_table_schema(self, table_name: str, sqlite_conn, schema: dict) -> bool:
        ...
        try:
            ...
            if not self._create_table_with_constraints(table_name, sqlite_conn, schema):
                ...
                sqlite_conn.commit()
                logging.error(
                    f"Failed to create new table {table_name}, restored from backup"
                )
                return False
            ...
            if primary_key:
                ...
                sqlite_cursor.execute(
                    f"SELECT {columns_str} FROM `{backup_table}` ORDER BY `{primary_key}`"
                )
                ...
                if migrated_count < original_count:
                    ...
                    logging.warning(
                        f"Migration of {table_name}: removed {duplicates_removed} duplicate rows"
                    )
                else:
                    logging.info(
                        f"Migration of {table_name}: all {migrated_count} rows migrated successfully"
                    )
            else:
            ...
            logging.info(
                f"Successfully migrated table {table_name} to include PRIMARY KEY constraints"
            )
            return True
        ...
        try:
            with DatabaseConnectionManager(
                ...
                with sqlite3.connect(self._dst_path, timeout=30.0) as sqlite_conn:
                    ...
                    logging.info(
                        f"Starting incremental backup for {len(all_db_tables)} tables"
                    )
                    ...
                    for table_name in all_db_tables:
                        try:
                            if table_name in self._TABLE_WITHOUT_KEY:
                                ...
                                logging.debug(
                                    f"Updating table without ID: {table_name}"
                                )
                                self._update_table_without_ID(
                                    table_name, mysql_conn, sqlite_conn
                                )
                            elif table_name.startswith("ROI_"):
                                # ROI tables: ROI_1, ROI_2, etc.
                                logging.debug(f"Updating ROI table: {table_name}")
                                self._update_table_with_ID(
                                    table_name, mysql_conn, sqlite_conn
                                )
                                ...
                                self._update_table_with_ID(
                                    table_name, mysql_conn, sqlite_conn, dump_csv=True
                                )
                            elif table_name in [
                                "START_EVENTS",
                                "IMG_SNAPSHOTS",
                            ...
                            logging.warning(
                                f"MySQL error updating table {table_name}: {e}"
                            )
                        except sqlite3.Error as e:
                            logging.warning(
                                f"SQLite error updating table {table_name}: {e}"
                            )

                    logging.info("Completed incremental backup for all tables")

        except Exception as e:
            logging.error(f"Error during database backup operation: {e}")
            raise

    def _update_table_with_ID(
        self, table_name: str, mysql_conn, sqlite_conn, dump_csv: bool = False
    ):
        """
        ...
        try:
            ...
            logging.warning(
                f"Table {table_name} doesn't exist locally, starting from beginning..."
            )
            # Table doesn't exist, start incremental backup from ID 0
            current_max_id = 0

        logging.debug(
            f"Table {table_name}: Starting incremental backup from ID {current_max_id}"
        )
        ...
        while True:
            ...
            logging.debug(
                f"Table {table_name}: Retrieved {rows_count} rows starting after ID {current_max_id}"
            )
            ...
            if rows_count > 0:
                ...
                rows_inserted = (
                    sqlite_cursor.rowcount if sqlite_cursor.rowcount > 0 else rows_count
                )
                ...
                if rows_inserted < rows_count:
                    ...
                    logging.warning(
                        f"Table {table_name}: Skipped {duplicates_skipped} duplicate rows during incremental backup"
                    )
                ...
                logging.debug(
                    f"Table {table_name}: Inserted {rows_count} rows, new max_id = {current_max_id}"
                )
        ...
        if total_inserted > 0:
            logging.info(
                f"Table {self._remote_db_name}.{table_name}: Incremental backup completed - inserted {total_inserted} new records"
            )
        else:
            logging.debug(
                f"Table {self._remote_db_name}.{table_name}: No new records to backup"
            )
    ...
    def _update_table_without_ID(self, table_name: str, mysql_conn, sqlite_conn):
        ...
        for row in remote_rows:
            ...
            sqlite_cursor.execute(
                f"SELECT 1 FROM `{table_name}` WHERE {where_conditions} LIMIT 1", row
            )
        ...
        if inserted_count > 0:
            logging.info(
                f"Table {table_name}: Row-by-row sync completed - inserted {inserted_count} new rows, skipped {skipped_count} duplicates"
            )
        else:
            logging.debug(
                f"Table {table_name}: No new rows to insert, {skipped_count} rows already present"
            )
