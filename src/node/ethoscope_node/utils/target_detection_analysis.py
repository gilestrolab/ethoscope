...
class TargetDetectionAnalyzer:
    ...
    def __init__(
        self, base_path: str = "/ethoscope_data/various/target_detection_logs"
    ):
        """
    ...
    def analyze_detection_logs(self, days_back: int = 30) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            "lighting_analysis": self._analyze_lighting_conditions(
                failed_data, success_data
            ),
        ...
        self.logger.info(
            f"Analyzed {len(failed_data)} failed and {len(success_data)} successful detections"
        )
        return analysis
    ...
    def _load_detection_data(
        ...
        for metadata_file in data_dir.glob("*_metadata.json"):
            try:
                ...
                self.logger.warning(
                    f"Skipping invalid metadata file {metadata_file}: {e}"
                )
    ...
    def _analyze_failure_patterns(self, failed_data: List[Dict]) -> Dict[str, Any]:
        ...
        for failure in failed_data:
            ...
            targets_found_counts[targets_found] = (
                targets_found_counts.get(targets_found, 0) + 1
            )
        ...
        most_common_targets_found = (
            max(targets_found_counts.items(), key=lambda x: x[1])
            if targets_found_counts
            else (0, 0)
        )
    ...
    def _analyze_device_performance(
        ...
        return {
            ...
            "problematic_devices": sorted(
                problematic_devices, key=lambda x: x["success_rate"]
            ),
    ...
    def _generate_recommendations(
        ...
        if not failed_data:
            recommendations.append(
                "No failures detected in the analyzed period - system performing well"
            )
        ...
        if failure_rate > 0.3:
            recommendations.append(
                f"High failure rate detected ({failure_rate:.1%}). Consider systematic review of lighting setup"
            )
        ...
        if dark_failures > len(failed_data) * 0.3:
            recommendations.append(
                "Many failures due to low lighting. Increase illumination or check for obstructions"
            )

        if bright_failures > len(failed_data) * 0.3:
            recommendations.append(
                "Many failures due to excessive brightness. Reduce illumination or add diffusion"
            )

        # Contrast issues
        low_contrast_failures = sum(
            1
            for f in failed_data
        ...
        if low_contrast_failures > len(failed_data) * 0.3:
            recommendations.append(
                "Low contrast contributing to failures. Check target visibility and arena cleanliness"
            )

        # Partial detection patterns
        partial_detections = sum(
            1 for f in failed_data if 0 < f.get("targets_found", 0) < 3
        )
        if partial_detections > len(failed_data) * 0.5:
            recommendations.append(
                "Many partial detections (1-2 targets found). Check target placement and visibility"
            )
        ...
        if zero_detections > len(failed_data) * 0.3:
            recommendations.append(
                "Many complete detection failures. Verify targets are present and properly positioned"
            )
    ...
    def generate_failure_report(self, days_back: int = 7) -> str:
        ...
        report_filename = (
            f"target_detection_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
    ...
    def export_dataset_for_training(
        ...
        for subdir, stat_key in [
            ("success", "success_exported"),
            ...
            for image_file in image_files:
                ...
                metadata_file = image_file.with_name(
                    image_file.name.replace("_original.png", "_metadata.json")
                )
