...
from ethoscope_node.auth.middleware import get_current_user
...
class AuthAPI(BaseAPI):
    ...
    def register_routes(self):
        ...
        self.app.route("/auth/sessions/<username>", method="DELETE")(
    ...
    def _login(self):
        ...
        try:
            ...
            # Debug logging
            self.logger.info(
                f"Login attempt - username: '{username}', pin length: {len(pin)}"
            )
    ...
    def _get_session(self):
        ...
        try:
            ...
            try:
                ...
                self.logger.warning(
                    f"Could not access authentication configuration: {config_error}"
                )
                auth_enabled = (
                    False  # Default to disabled if we can't determine the setting
                )
    ...
    def _change_pin(self):
        ...
        try:
            ...
            if not self.database.verify_pin(username, current_pin):
                self.logger.warning(
                    f"PIN change failed - invalid current PIN for user: {username}"
                )
            ...
            if not hashed_pin:
                ...
                return {"success": False, "message": "Failed to process new PIN"}
    ...
    def _get_all_sessions(self):
        ...
        try:
            ...
            return {"success": True, "sessions": sessions, "total_count": len(sessions)}
    ...
    def _terminate_user_sessions(self, username):
        ...
        try:
            ...
            if success:
                self.logger.info(
                    f"Terminated all sessions for user: {username} (by {current_username})"
                )
                return {
                    "success": True,
