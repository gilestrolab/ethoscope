...
class DeviceAPI(BaseAPI):
    ...
    def register_routes(self):
        ...
        self.app.route("/devices/cleanup-busy", method="POST")(
            self._cleanup_busy_devices
        )
        ...
        self.app.route("/device/<id>/machineinfo", method="GET")(
            self._get_device_machine_info
        )
        self.app.route("/device/<id>/machineinfo", method="POST")(
            self._set_device_machine_info
        )
        self.app.route("/device/<id>/module", method="GET")(self._get_device_module)
        self.app.route("/device/<id>/user_options", method="GET")(
            self._get_device_options
        )
        self.app.route("/device/<id>/videofiles", method="GET")(
            self._get_device_videofiles
        )
        ...
        # Device operations
        self.app.route("/device/<id>/backup", method="GET")(
            self._get_device_backup_info
        )
        ...
        self.app.route("/device/<id>/controls/<instruction>", method="POST")(
            self._post_device_instructions
        )
        ...
        self.app.route("/device/<id>/batch-critical", method="GET")(
            self._get_device_batch_critical
        )
    ...
    def _get_devices(self):
        ...
        include_inactive = (
            self.get_query_param("include_inactive", "").lower() == "true"
        )
    ...
    def _retire_inactive_devices(self):
        ...
        try:
            ...
            busy_cleaned_count = self.database.cleanup_stale_busy_devices(
                timeout_minutes=10
            )
    ...
    def _set_device_machine_info(self, id):
        ...
        if response.get("haschanged", False):
            try:
                ...
                self.logger.info(
                    f"SSH key setup for device {id}: {'successful' if ssh_success else 'failed'}"
                )
            except Exception as e:
    ...
    def _post_device_instructions(self, id, instruction):
        ...
        if instruction == "start":
            ...
            try:
                ...
                logging.info(
                    f"DEBUG: Node server sending start instruction to device {id}"
                )
                ...
                if "experimental_info" in data_dict:
                    ...
                    if (
                        "arguments" in exp_info
                        and "database_to_append" in exp_info["arguments"]
                    ):
                        logging.info(
                            f"DEBUG: Found database_to_append in node server: {exp_info['arguments']['database_to_append']}"
                        )
                    else:
                        logging.warning(
                            "DEBUG: database_to_append NOT found in node server data"
                        )
            except Exception as e:
