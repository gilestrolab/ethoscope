...
class TunnelUtils(BaseAPI):
    ...
    def register_routes(self):
        ...
        self.app.route("/tunnel/config", method="POST")(
            self._update_tunnel_config_route
        )
    ...
    def update_tunnel_environment(self) -> None:
        ...
        try:
            if not self.config:
                self.logger.warning(
                    "No configuration available for tunnel environment update"
                )
                return
            ...
            if tunnel_token:
                ...
                self.logger.info(
                    f"Updated tunnel environment file and UPDATE_SERVICE_URL to {update_url}"
                )
            else:
                self.logger.debug(
                    "No tunnel token configured, skipping environment file update"
                )
    ...
    def get_hostname_aware_redirect_url(
        self, current_host: str, fallback_url: str
    ) -> str:
        """
        ...
        try:
            ...
            hostname = (
                current_host.split(":")[0] if ":" in current_host else current_host
            )
    ...
    def get_tunnel_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if service_status == "active":
                with os.popen(
                    f"{systemctl} status ethoscope_tunnel --no-pager -l"
                ) as result:
    ...
    def toggle_tunnel(self, enabled: bool) -> Dict[str, Any]:
        ...
        self.config.update_tunnel_config(
            {"enabled": enabled, "status": "connecting" if enabled else "disconnected"}
        )
        ...
        try:
            ...
            if not tunnel_token and enabled:
                return {
                    "success": False,
                    "error": "Tunnel token not configured",
                    "enabled": False,
                }
