...
class AuthMiddleware:
    ...
    def __init__(self, database, config):
        ...
        self._login_attempts = (
            {}
        )  # {username: {'count': int, 'last_attempt': timestamp}}
    ...
    def check_rate_limit(self, username: str, client_ip: str) -> bool:
        ...
        user_attempts = self._login_attempts.get(
            username, {"count": 0, "last_attempt": 0}
        )
    ...
    def record_failed_attempt(self, username: str, client_ip: str):
        ...
        self.logger.info(
            f"Recorded failed login attempt for {username} from {client_ip}"
        )
    ...
    def authenticate_user(
        ...
        if not self.database.verify_pin(username, pin):
            self.record_failed_attempt(username, client_ip)
            self.logger.warning(
                f"Authentication failed - invalid PIN for user: {username}"
            )
            return None
