...
def home():
    ...
    if gbw is None:
        return json.dumps(
            {"status": "initializing", "error": "Backup wrapper not initialized"},
            indent=2,
        )
    ...
    try:
        with gbw._lock:
            ...
            active_jobs = sum(
                1
                for status in gbw.backup_status.values()
            ...
            try:
                ...
                current_devices = (
                    gbw.find_devices() if hasattr(gbw, "find_devices") else []
                )
            ...
            device_stats = {
                ...
                "devices_with_status": (
                    len(gbw.backup_status) if hasattr(gbw, "backup_status") else 0
                ),
            ...
            status_response = {
                ...
                "thread_status": {
                    ...
                    "is_running": (
                        gbw.is_running() if hasattr(gbw, "is_running") else False
                    ),
                    "stop_event_set": (
                        gbw._stop_event.is_set()
                        if hasattr(gbw, "_stop_event")
                        else None
                    ),
                },
                "health_status": (
                    gbw.get_health_status() if hasattr(gbw, "get_health_status") else {}
                ),
            }
            ...
            for device_id, backup_status in gbw.backup_status.items():
                if hasattr(backup_status, "__dict__"):
                    ...
                    status_dict[device_id] = {
                        ...
                        "data_duplication": getattr(
                            backup_status, "data_duplication", False
                        ),
                    }
                else:
...
def setup_logging(debug=False):
    ...
    for logger_name in [
        "BackupWrapper.GenericBackupWrapper",
        "BackupClass",
        "VideoBackupClass",
    ]:
...
def parse_arguments():
    ...
    parser.add_option(
        ...
        default=False,
        action="store_true",
        help="Set Safe mode ON (currently unused)",
    )
    parser.add_option(
        "-p",
        "--port",
...
def create_device_info_from_backup(ethoscope_name, host, backup_filename):
    ...
    try:
        ...
        logging.warning(
            f"Could not extract device ID from backup filename {backup_filename}: {e}"
        )
...
def force_backup_ethoscopes(ethoscope_list):
    ...
    logging.info(
        f"Starting direct database backup for {len(ethoscope_list)} targets..."
    )
    ...
    for identifier_type, value in ethoscope_list:
        try:
            ...
            logging.info(
                f"MariaDB database confirmed for {ethoscope_name} - starting forced backup job..."
            )

            # Initiate backup using the synthetic device info
            logging.info(
                f"Initiating forced MariaDB backup job for {ethoscope_name}..."
            )
            success = gbw.initiate_backup_job(device_info)

            if success:
                logging.info(
                    f"=== BACKUP COMPLETED SUCCESSFULLY for {ethoscope_name} ==="
                )
            else:
            ...
            logging.error(
                "This could mean the ethoscope is offline or database is not accessible"
            )
            sys.exit(1)
        except ValueError as e:
            logging.error(f"Database query failed for {ethoscope_name}: {e}")
            logging.error(
                "This could mean no experiment is running or backup_filename is not set"
            )
            sys.exit(1)
        except Exception as e:
...
def backup_wrapper_context(results_dir, node_address):
    ...
    try:
        logging.info(
            f"Creating GenericBackupWrapper with results_dir='{results_dir}', node_address='{node_address}'"
        )
        ...
        if gbw:
            ...
            if gbw.is_alive() or hasattr(gbw, "_started") and gbw._started.is_set():
                logging.info(
                    "Waiting for GenericBackupWrapper to join (timeout=10s)..."
                )
                gbw.join(timeout=10)
                if gbw.is_alive():
    ...
    try:
        ...
        with backup_wrapper_context(results_dir, options.server) as wrapper:
            if ethoscope_list:
                ...
                time.sleep(2)
                logging.info(
                    f"Backup worker thread status: alive={wrapper.is_alive()}, running={wrapper.is_running()}"
                )
                ...
                server_thread.start()
                logging.info(
                    f"HTTP status server started in background thread on port {options.port}"
                )
