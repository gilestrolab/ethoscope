        ...
        return {
            ...
            "combined": {
                ...
                "total_size_human": format_bytes(
                    total_results_bytes + total_videos_bytes
                ),
            },
        }
...
class UnifiedBackupWrapper(GenericBackupWrapper):
    ...
    def __init__(
        ...
        super().__init__(
            results_dir, node_address, video=False, max_threads=max_threads
        )
        ...
        self._logger = logging.getLogger("UnifiedBackupWrapper")
    ...
    def initiate_backup_job(self, device_info: dict) -> bool:
        ...
        try:
            ...
            self._logger.info(
                f"=== INITIATING UNIFIED BACKUP JOB for {device_name} (ID: {device_id}) ==="
            )
            self._logger.info(
                f"Device details: IP={device_ip}, Status={device_status}, Backup types={backup_types}"
            )

            # Create unified backup job
            self._logger.info(
                f"Creating UnifiedRsyncBackupClass for device {device_id}"
            )
            ...
            self._logger.info(
                f"Unified backup job object created successfully for device {device_id}"
            )
            ...
            self._logger.info(
                f"Starting unified backup execution for device {device_id}"
            )
            ...
            if success:
                self._logger.info(
                    f"=== UNIFIED BACKUP JOB COMPLETED SUCCESSFULLY for {device_name} in {job_elapsed_time:.1f}s ==="
                )
            else:
                self._logger.error(
                    f"=== UNIFIED BACKUP JOB FAILED for {device_name} after {job_elapsed_time:.1f}s ==="
                )
            ...
            job_elapsed_time = time.time() - job_start_time
            self._logger.error(
                f"=== UNIFIED BACKUP JOB CRASHED for {device_name} after {job_elapsed_time:.1f}s ==="
            )
...
def main():
    ...
    try:
        ...
        parser.add_option(
            ...
            dest="unified",
            default=True,
            help="Use unified rsync for both results and videos (default)",
            action="store_true",
        )
        parser.add_option(
            "--results-only",
            dest="results_only",
            default=False,
            help="Backup only database/results files",
            action="store_true",
        )
        parser.add_option(
            "--videos-only",
        ...
        # Configuration
        RESULTS_DIR = (
            option_dict["results_dir"] or CFG.content["folders"]["results"]["path"]
        )
        VIDEOS_DIR = (
        ...
        BACKUP_RESULTS = not option_dict[
            "videos_only"
        ]  # Default True unless videos-only
        BACKUP_VIDEOS = not option_dict[
            "results_only"
        ]  # Default True unless results-only
        ...
        if ETHO_TO_BACKUP:
                ...
                for device in gbw.find_devices():
                    if device["name"] == ("ETHOSCOPE_%03d" % ethoscope):
                        ...
                        if sqlite_databases and len(sqlite_databases) > 0:
                            has_sqlite = True
                            print(
                                f"SQLite databases found (new format): {list(sqlite_databases.keys())}"
                            )
                        ...
                        if not has_sqlite:
                            ...
                            sqlite_exists = database_info.get("sqlite", {}).get(
                                "exists", False
                            )
                            if sqlite_exists or active_type == "sqlite":
                                has_sqlite = True
                                print(
                                    f"SQLite database found (old format): active_type={active_type}"
                                )
            ...
            server_address = (
                "",
                8093,
            )  # Serve on all interfaces at port 8093 (different from video backup)
