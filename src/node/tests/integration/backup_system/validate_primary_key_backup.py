    ...
    def test_schema_detection(self):
        ...
        try:
            ...
            for table_name, schema in test_schema.items():
                ...
                if has_pk:
                    pk_col_info = next(
                        (col for col in schema["columns"] if col["name"] == pk_column),
                        None,
                    )
                    if pk_col_info and pk_col_info["is_primary"]:
                        ...
                        self.test_results.append(
                            f"Schema detection failed for {table_name}"
                        )
                else:
    ...
    def test_table_creation_with_constraints(self):
        ...
        try:
            with sqlite3.connect(test_db_path) as conn:
                ...
                cursor.execute(
                    "INSERT INTO test_roi (id, t, x, y) VALUES (1, 100, 10, 20)"
                )

                try:
                    cursor.execute(
                        "INSERT INTO test_roi (id, t, x, y) VALUES (1, 200, 30, 40)"
                    )
                ...
                cursor.execute(
                    "INSERT OR IGNORE INTO test_roi (id, t, x, y) VALUES (1, 300, 50, 60)"
                )
    ...
    def test_table_migration(self):
        ...
        try:
            with sqlite3.connect(test_db_path) as conn:
                ...
                cursor.executemany(
                    "INSERT INTO test_table (id, data) VALUES (?, ?)", test_data
                )
                conn.commit()
                ...
                cursor.execute(
                    f"CREATE TABLE {backup_table} AS SELECT * FROM test_table"
                )
                ...
                cursor.execute(
                    f"INSERT OR IGNORE INTO test_table SELECT * FROM {backup_table} ORDER BY id"
                )
                ...
                if final_count == unique_ids and final_count < initial_count:
                    ...
                    print(
                        f"✓ Migration successful: {duplicates_removed} duplicates removed"
                    )
                    print(f"  Final rows: {final_count} (all unique)")
                    self.test_results.append("Table migration: PASSED")
                else:
                    print(
                        f"✗ Migration failed: final={final_count}, unique={unique_ids}"
                    )
    ...
    def test_duplicate_prevention(self):
        ...
        try:
            with sqlite3.connect(test_db_path) as conn:
                ...
                cursor.executemany(
                    "INSERT OR IGNORE INTO backup_test VALUES (?, ?, ?)", batch2
                )
                ...
                if (
                    ...
                    print(
                        f"✓ Duplicate prevention working: {total_rows} rows, all unique"
                    )
                    self.test_results.append("Duplicate prevention: PASSED")
                else:
                    print(
                        f"✗ Duplicate prevention failed: {total_rows} rows, {unique_ids} unique"
                    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test PRIMARY KEY backup implementation"
    )
    parser.add_argument(
