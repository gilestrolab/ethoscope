...
class TestCachePerformanceRealistic(unittest.TestCase):
    ...
    def _create_simulated_files(self, count, age_distribution=None):
        ...
        for i in range(recent_count):
            ...
            age_seconds = (
                i * (7 * 24 * 60 * 60) // recent_count
            )  # Spread across last week
    ...
    def test_cache_performance_1k_files(self):
        ...
        # Record metrics
        self.performance_metrics["Cache 1K Files"] = {
        ...
        self.assertLess(
            save_time, 2.0, f"Save 1K files should be under 2s, took {save_time:.3f}s"
        )
        self.assertLess(
            load_time, 1.0, f"Load 1K files should be under 1s, took {load_time:.3f}s"
        )
    ...
    def test_cache_performance_10k_files(self):
        ...
        # Record metrics
        self.performance_metrics["Cache 10K Files"] = {
            "file_count": file_count,
            "save_time": save_time,
            "load_time": load_time,
            "files_per_second_save": file_count / save_time,
        ...
        self.assertLess(
            save_time,
            10.0,
            f"Save 10K files should be under 10s, took {save_time:.3f}s",
        )
        self.assertLess(
            load_time, 5.0, f"Load 10K files should be under 5s, took {load_time:.3f}s"
        )
    ...
    def test_filesystem_scan_vs_cache_performance(
        self, mock_getmtime, mock_getsize, mock_exists, mock_glob
    ):
        ...
        h264_files = [
            os.path.join(device_path, filename) for filename in simulated_files.keys()
        ]

        mock_glob.return_value = h264_files
        mock_exists.return_value = True
        mock_getsize.side_effect = lambda path: simulated_files.get(
            os.path.basename(path), {}
        ).get("size_bytes", 0)
        mock_getmtime.side_effect = lambda path: simulated_files.get(
            os.path.basename(path), {}
        ).get("mtime", time.time())
        ...
        self.assertLess(
            ...
            filesystem_scan_time * 0.5,
            "Cache-optimized scan should be at least 50% faster",
        )
    ...
    def test_concurrent_cache_access_simulation(self):
        ...
        for device_id in devices.keys():
            loaded_caches[device_id] = _load_video_cache(
                device_id, self.video_directory
            )
...
class TestCacheRobustness(unittest.TestCase):
    ...
    def test_cache_permission_handling(self):
        ...
        try:
            ...
            self.fail(
                f"Cache save should handle permission errors gracefully, got: {e}"
            )
        finally:
...
if __name__ == "__main__":
    ...
    # Add test classes
    test_classes = [TestCachePerformanceRealistic, TestCacheRobustness]
