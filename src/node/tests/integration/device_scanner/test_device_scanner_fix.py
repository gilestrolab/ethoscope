...
def test_backup_status_update():
    ...
    if device._info.get("backup_status") == 75.5:
        ...
        print(
            f"✗ Direct backup status: Expected 75.5, got {device._info.get('backup_status')}"
        )
        return False
    ...
    with patch.object(device, "_logger") as mock_logger:
        try:
            ...
            if len(warning_calls) == 0:
                print(
                    "✓ Warning eliminated: No 'backup filename available' warning produced"
                )
                return True
            else:
                print(
                    f"✗ Warning still present: Found {len(warning_calls)} warning calls"
                )
                return False
