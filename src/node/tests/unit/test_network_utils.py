...
class TestGetLocalIpAddresses:
    ...
    def test_handles_interfaces_without_ipv4(self, mock_ifaddresses, mock_interfaces):
        ...
        mock_interfaces.return_value = ["eth0", "eth1"]
        mock_ifaddresses.side_effect = [{}, {2: [{"addr": "192.168.1.50"}]}]  # no IPv4
...
class TestNetworkIntegration:
    ...
    def test_pattern_matches_detected_network(self):
        ...
        if network:
            ...
            test_ip = str(
                list(net.hosts())[0] if list(net.hosts()) else net.network_address
            )
            ...
            for _i, (pattern_part, ip_part) in enumerate(
                zip(pattern_parts, ip_parts, strict=False)
            ):
                if pattern_part != "*":
                    assert (
                        pattern_part == ip_part
                    ), f"Pattern {pattern} should match IP {test_ip}"
        else:
