...
def clean_h264_files(root_path, extension="h264"):
    """
    Delete all .h264 files in folders that contain an .mp4 file.
    """
    all_folders = [x[0] for x in os.walk(root_path)]
    ...
    for folder in folders_with_mp4:
        ...
        if h264_files:
            folder_cleaned = 0
            folder_size_cleaned = 0
            for file in h264_files:
                file_size = os.path.getsize(file)
                os.remove(file)
                folder_cleaned += 1
                folder_size_cleaned += file_size
            
            total_cleaned += folder_cleaned
            total_size_cleaned += folder_size_cleaned
            print(f"Cleaned {folder_cleaned} files ({sizeof_fmt(folder_size_cleaned)}) in {folder}")

    print(f"\nTotal cleaned: {total_cleaned} files")
    print(f"Total space freed: {sizeof_fmt(total_size_cleaned)}")
...
if __name__ == '__main__':
    ...
    parser.add_option("--clean", dest="clean", default=False, help="Clean h264 files in folders with MP4s", action="store_true")
    ...
    if option_dict['list']:
        ...
        print("\n".join(l))
        print(f"Found {len(l)} folders with mp4 files")
        os.sys.exit()
    
    if option_dict['clean']:
        clean_h264_files(option_dict['path'], extension=option_dict['extension'])
        os.sys.exit()
