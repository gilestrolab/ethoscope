...
sleep monitors and sleep deprivators network. 
...
To connecto to Node with ssh: ssh root@192.169.123.1 or ssh node@192.169.123.1

If you prefer to use the Ethernet you need to know the assigned address by the
external provider. 

## Connecting to a Device
The devices are only conneced to the wifi subnetwork so this is the only way to
access to them. 
The IP address is assigned dinamically and can be discovered through node server
software. 
...
This bare repo is the one that use the own node and devices to update itselfs.
It is installed inside /var/pySolo-Video.git

The node has another local repository (not bare) which contains the actual data, 
it is syncronised with the bare repository in /var/ not with Github. This is found 
